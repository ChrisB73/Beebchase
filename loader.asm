...
puttext "bootfile.txt","!Boot",0

;putbasic "chaseintro.bas","Intro"        ; To allow it to be compressed
...
ORG &1800                                 ; Main code is from 
GUARD &2FFC                               ; Main code JMP is at 2FFD
...
.loaderstart
    ...
    jsr oswrch
    lda #1
    jsr oswrch              ; Change to mode 1

    ; Change colours to black
    ldx #3
.blacklp
    lda #19
    jsr oswrch
   ...
   jsr decompressmain
    ...
    lda #19
    jsr &fff4  ; Wait for vsync

   jmp &3000-3 ; Jump to init code
...
NIBCOUNT = &fe                          ; zero-page location for temp offset
...
.REPMATCH_OR_16_BIT                      ; rep-match or 16 bit offset
   ;;ASL                                  ; XYZ=111?
   BMI REP_MATCH                        ; reuse pious offset if so (rep-match)
...
PRINT "Loader Space",&2FFC-P%,~P%

save "Chase",loaderstart,P%